...
from ..extract_states import get_all_attention_maps
...
@dataclass
class CondEnt(HallucinationDetectionMethod):
    ...
    def transform(self, X: pd.DataFrame) -> list[list[float]]:
        ...
        for i in range(len(X)):
            scores.append(self.calc_condent(X.iloc[i], self.llm_model))
        return scores

    def fit(self, *args, **kwargs) -> "CondEnt":
        """Return the instance of this class."""
        return self

    def predict_score(self, X: list[list[float]]) -> np.ndarray:
        """Perform inference.
        ...
        logger.info("Average CondEnt is used as a hallucination score.")
    ...
    def compute_condent(
        ...
        with ThreadPool(processes=min(mp.cpu_count(), 4)) as pool:
